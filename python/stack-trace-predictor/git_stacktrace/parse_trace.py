...
class JavaTraceback(Traceback):
    ...
    def extract_traceback(self, lines):
        if not lines[0].lstrip().startswith('at'):
            self.header = lines[0] + '\n'
        lines = [line.strip() for line in lines if line.lstrip().startswith('at')]
        extracted = []
        for line in lines:
    ...
    def _extract_line(self, line_string):
        ...
        container_index = line_string.rfind(' ~')
        if container_index != -1:
            line_string = line_string[:container_index]
        container_index = line_string.rfind(' [')
        if container_index != -1:
            line_string = line_string[:container_index]
