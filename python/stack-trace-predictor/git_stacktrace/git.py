...
class GitFile(object):
    ...
    # Experimental
    UNKNOWN = 'F'
    VALID = frozenset([ADDED, DELETED, MODIFIED, COPY_EDIT, RENAME_EDIT, TYPE_CHANGED, UNKNOWN])

    def __init__(self, filename, state=None):
        self.filename = filename
        if state.upper() not in GitFile.VALID:
...
def convert_since_until(since, until, branch=None):
    cmd = ['git', 'log', '--pretty=%H']
    if since is not None:
        if not since.startswith("\"") or not since.endswith("\""):
            since = "\"%s\"" % since
        cmd.append("--since=%s" % since)
    if until is not None:
        if not until.startswith("\"") or not until.endswith("\""):
            until = "\"%s\"" % until
    ...
    lines = data.splitlines()
    if len(lines) == 0:
        raise Exception("Didn't find any commits in 'since-until' range, try updating your git repo")
